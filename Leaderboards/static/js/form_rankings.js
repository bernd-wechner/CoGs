...
// Sessions structure).
//
// THis function is defined in the Django form template as it needs access to 
// Django context variables which this JS module does not. 
...
// We ignore disabled elements as they won't be submitted anyhow
const qs_tids 			  = `:not(:disabled)[id^='${id_prefix}${team_prefix}'][id$='-id']`;
const qs_rids 			  = `:not(:disabled)[id^='${id_prefix}${rank_prefix}'][id$='-id']`;
const qs_pids 			  = `:not(:disabled)[id^='${id_prefix}${performance_prefix}'][id$='-id']`;

const qs_tdels 			  = `:not(:disabled)[id^='${id_prefix}${team_prefix}'][id$='-DELETE']`;
const qs_rdels 			  = `:not(:disabled)[id^='${id_prefix}${rank_prefix}'][id$='-DELETE']`;
const qs_pdels 			  = `:not(:disabled)[id^='${id_prefix}${performance_prefix}'][id$='-DELETE']`;

const qs_ranks 			  = `:not(:disabled)[id^='${id_prefix}${rank_prefix}'][id$='-rank']`;
const qs_rscores 		  = `:not(:disabled)[id^='${id_prefix}${rank_prefix}'][id$='-score']`;
const qs_pscores 		  = `:not(:disabled)[id^='${id_prefix}${performance_prefix}'][id$='-score']`;

const qs_pplayers 		  = `:not(:disabled)[id^='${id_prefix}${performance_prefix}'][id$='-player']`;
const qs_rplayers 		  = `:not(:disabled)[id^='${id_prefix}${rank_prefix}'][id$='-player']`;
...
const id_show_tie_breakers_i = "ShowTieBreakers_Individual";							// ID of the checkbox for display tie breakers
const id_show_tie_breakers_t = "ShowTieBreakers_Team";									// ID of the checkbox for display tie breakers
let   id_show_tie_breakers   = null;												    // ID of the checkbox for display tie breakers
...
let   id_tbl_visbl 		  = undefined;													// The visible (in use) table (id_tbl_teams or id_tbl_players)
...
function disable_ranks_scores_as_needed(div) {
	...
	// has lapsed in the task of rank reconiliation. Rank reconciliation is the
	// process of ensuring that ranks, rank scores and performance scores are
...
function configureGame() {
	...
	else if (game_team_play) show_team_play = true;
	...
	if (show_team_play) {
		...
		showDiv("teams");
		// The game supports team play, we need the switch
		showhideTeamSwitch(true);
	} else {
		if (team_switch.checked) {
		...
		showDiv("indiv");
		// The game does not support team play, we don't need the switch
		showhideTeamSwitch(false);
	}
...
// The tables have columns for ranks and scores. Depending on the game.scoring configuration
...
function manage_rank_ties(session) {
	...
	if (show_tiebreakers) reconcile_ranks(session); // if the tie_breakers column (ranks) is visible, reconcile the ranks
}

// Acts on the visible table (Team or Individual session)
// and examines the scores to see if ranks need adjusting
// as tie breakers.
function reconcile_ranks(session, display) {
    ...
    // If no session was provided we cna liberally rewrite all the ranks to reflect the 
	// scores entered.
	//
	...
	// Session as Session.provided. Quite distinct from the local session (which may have been
	...
	let i = 1;
	let prev_rank = 1;
	let prev_score = Number.MIN_SAFE_INTEGER;
	
	// If a seesion is provided and added new players and ranks then the new rank may be inserted and all later ranks need to be shunted one higher
	let shunt = 0;
	// If we are inserting into a tied group we don't want to shunt any subsequent provided rnaks that are part of this tie group
	let last_inserted_rank = null; 
	...
	for (let rid of score_sorted_rids) {
		...
		console.log(`Reconciling ranks:	${rid}	${score} => ${rank}`);
		if (Session.provided) {
			const provided_rank = Session["rRanks"][rid];
			const provided_rank_valid = isValidRank(provided_rank);
			if (!provided_rank_valid) {
				session["rRanks"][rid] = rank;
				shunt++;
				last_inserted_rank = rank;
			}
			else if (shunt && provided_rank != last_inserted_rank) 
				session["rRanks"][rid] = provided_rank + shunt;
			
			console.log(`\tprovided rank: ${provided_rank}	valid: ${provided_rank_valid} assigned: ${session["rRanks"][rid]}`);
		} else {
			session["rRanks"][rid] = rank;
		}
		i++;
...
// A robust way to find a widget within a row when we don't know the index, 
// but we know the Formset Prefix and the Field Name.
function getWidgetInRow(row, id) {
	if (id.startsWith(id_prefix)) {
		const parts = id.slice(id_prefix.length).split(form_number);
		if (parts.length == 2) {
			const model = parts[0];
			const field = parts[1];
			const selector = `[id^="${id_prefix}${model}"][id$="${field}"]`;
			return row.querySelector(selector);
		} else {
			console.warn(`getWidgetInRow: Could not find placeholder "${form_number}" in "${id}".`);
			return null;			
		}
	} else {
	        console.warn(`getWidgetInRow: Supplied ID "${id}" lacks required prefix "${id_prefix}". Bailing.`);
	        return null;
	}	
}
...
/**
 * Analyzes a row to see if all Django Formset elements share the same index.
 * Returns: 
 * - The index (Number) if they all agree.
 * - A Set of indices if they disagree (highly unexected edge case and bug!).
 * - null if no widgets are found.
 */
function getFormNumberFromRow(rowEl) {
    const indices = new Set();
    
    // List the IDs we want to check for index consistency
    const checkIds = [id_rid, id_pid, id_player, id_player_copy, id_rscore, id_rank, id_weight];

    checkIds.forEach(id => {
        const el = getWidgetInRow(rowEl, id);
        const idx = getFormNumber(el);
        if (idx !== null) {
            indices.add(idx);
        }
    });
    ...
    if (indices.size === 1) return Array.from(indices)[0]; // Return the single number
    
    return indices; // Return the Set for the highly unexpected edge cae of disagreement internally
}
...
function getNumberValue(element, default_value) {
    ...
    // Special Case: If it's a Select2 widget and the value is empty, 
    // it might be in the middle of a DAL/AJAX sync.
    if ((val === '' || val === null) && element.classList.contains('select2-hidden-accessible')) {
        // Use jQuery to peek into the Select2 internal state
        const s2Data = $(element).select2('data');
        if (s2Data && s2Data.length > 0) {
            val = s2Data[0].id;
        }
    }

    return (val === '' || val === null || isNaN(val)) ? default_value : Number(val);
}

/* ===================================================================================================
Table Management (Buiilding and reading player/team tables)
======================================================================================================*/

// Used when applying sessions to tables to ensure a sessions mentioned player appears in the DAL selector
function add_player(select2, player_id) {
	...
	if (Number(player_id)) {
	    ...
	    $.ajax({
			...
			// Avoid duplicate entries, only add the option if not already there
			if (select2.find(`option[value="${player_id}"]`).length === 0) {
...
function applySessionToRow(session, row, table_type) {
    ...
    switch (table_type) {
    	case TableType.Players: {
    	    ...
    	    const rank_id 	    = perf_id in pidRIDs ? pidRIDs[perf_id] : "";
			...
			const formNumber = getFormNumberFromRow(row);

			switch (typeof formNumber) {
				case "number":
					if (formNumber < pids.length) {
						// Populate the controls on the row with the session data
						rid.value 	 = rank_id
						...
						if (debug) console.log(`Row for form ${formNumber} has no session data to populate it with and was likely added after the session loaded`);
					}
				    break;
			    case "undefined":
					console.error(`[Row Integrity] No Django form widgets found in row ${row}.`);					
			        break;
			    case "object":
					const csv = Array.from(formNumber).join(", ");
			        console.error(
			            `%c[Row Integrity] Multiple Django Form indices [${indices}] found in row ${row}. `,
...
function RenderTable(template, entries, placein, entry_number, session) {
    ...
    if (session) {
		...
		const num_ranks = session["rIDs"].length; 
	    ...
	    for (let i = 1; i < rowsPresent; i += step) {
			...
			switch (typeof formNumber) {
				case "number":
					if (formNumber < num_ranks) { 
						if (debug) console.log(`Applying session data to row ${i} (form ${formNumber}) of ${rowsPresent}`);
						applySessionToRow(session, row, table_type);
					} else {
						if (debug) console.log(`Row ${i} (form ${formNumber}) has no session data to populate it with and was likely added after the session loaded`);
					}
				    break;
			    case "undefined":
					console.error(`[Row Integrity] No Django form widgets found in row ${i}.`);					
			        break;
			    case "object":
					const csv = Array.from(formNumber).join(", ");
		            console.error(
		                `%c[Row Integrity] Row ${i} contains multiple Django Form indices: [${indices}]. `,
		                "color: red; font-weight: bold;"
		            );
			        break;
			}	
    ...
    if (rowsNeeded < rowsPresent) {
        ...
        if (entries > 1) {
			...
			if (debug) console.log(`Chopping ${chop} rows from table of ${rowsPresent} rows`);
		...
		// Now let's work out how many steps of "adding" rows we need
        ...
        for (let i = 0; i < steps; i++) {
        	...
        	if (have_trash) {
				if (debug) console.log(`Popping row from trash.`);
				// Always take the last row from trash first.
				// Because we always add trash to the last row. 
				// So this creates a FIFO stack. Which is desireable. 	
				...
				if (debug) console.log(`Created new row and applying session data if it's available. Entry ${entry_id} (step ${i+1} of ${steps})`);
...
function removeRowGently(row) {
    return new Promise((resolve) => {
        ...
        // Safety fallback: if for some reason CSS fails to load, 
        // don't let the UI hang forever.
...
function adjustTable(element, session) {
	...
	// team_players are special in that they don't become siblings of the template
	// they have to go in cell of the Teams table that did.
    const placein = is_teamplayers ? $$(tr.id.replace("Body","DetailCell")): (template.closest('fieldset') || template).parentNode;
	...
	if (debug) table_integrity_debug(tableControl);	
...
/* ===================================================================================================
Session Management (convert between HTML and JS)
======================================================================================================*/
...
// Give a fieldset toggles the disabled attribute safely on all eelemtns in it 
function toggleTemplateFields(fieldsetId, shouldDisable) {
    const container = document.getElementById(fieldsetId);
    if (!container) return;

    // container.elements is a built-in HTML collection of ALL form controls 
    // inside a fieldset or form. It is the most robust way to find them.
    const controls = container.elements;

    for (let control of controls) {
        if (shouldDisable) {
            // Save state: true if currently disabled, false if enabled
            control.dataset.originalDisabled = control.disabled; 
            control.disabled = true;
        } else {
            // Restore state: only if we have a saved value
            const savedState = control.dataset.originalDisabled;
            if (savedState !== undefined) {
                // Convert string "true"/"false" back to actual boolean
                control.disabled = (savedState === "true");
...
function disableTemplateFields(fieldsetId) { toggleTemplateFields(fieldsetId, true); };
function restoreTemplateFields(fieldsetId) { toggleTemplateFields(fieldsetId, false); };

// Give a fieldset toggles the disabled attribute safely on all eelemtns in it 
function toggleField(fieldId, shouldDisable) {
	control = document.getElementById(fieldId);
    if (shouldDisable) {
        // Save state: true if currently disabled, false if enabled
        control.dataset.originalDisabled = control.disabled; 
        control.disabled = true;
...
function disableField(fieldId) { toggleField(fieldId, true); };
function restoreField(fieldId) { toggleField(fieldId, false); };
...
function OnSubmit(event) {
	...
	if ($$("templatePlayersTable")) disableTemplateFields("templatePlayersTableFields");
	if ($$("templateTeamsTable")) disableTemplateFields("templateTeamsTableFields");
	if ($$("templateTeamPlayersTable")) disableTemplateFields("templateTeamPlayersTableFields");

	// Remove any elements flagged as NoSubmit by class
	const killthem = document.getElementsByClassName("NoSubmit");
	for (let i = 0; i < killthem.length; i++) disableField(killthem[i].id);
	...
	if (team_play) {
		...
		// TODO: If I increase rows and don't fill them out this leaves empty rows, no player selected, and no score or rank. 
		//       We should in this case remove these form the submission! Or bounce with a warning perhaps, rather than guess. 
		//       We cna do that test client side, could write a function like the table intergity checker that returns a message 
		// 		 to write to the messages script and bail on hte sumbission. 
		...
		*/
		...
		if (print_debug_info) {
			// We're not submitting (this is triggered off a normal button) so we restore the templates
			submission_debug(div);
			if ($$("templatePlayersTable")) restoreTemplateFields("templatePlayersTableFields");
			if ($$("templateTeamsTable")) restoreTemplateFields("templateTeamsTableFields");
			if ($$("templateTeamPlayersTable")) restoreTemplateFields("templateTeamPlayersTableFields");
			
			// Restore any elements flagged as NoSubmit by class
			const restorethem = document.getElementsByClassName("NoSubmit");
			for (let i = 0; i < restorethem.length; i++) restoreField(restorethem[i].id);
		}		
		else
			...
			*/
			if (was_team_play) {
				...
				*/
				...
				// We may also have added new players during the edit. These will have empty rid and pid fields
				// or generic ones like id_3, id_4 etc. We remove those from the submission to prmpt the server 
				// side to create objects as needed. 
				for (let p = 0; p < num_players; p++) {
					const rid = getWidget(table, name_rid, p);
					const pid = getWidget(table, name_pid, p);

					if (rid && !isValidID(rid.value.trim())) rid.disabled = true;
				    if (pid && !isValidID(pid.value.trim())) pid.disabled=true;
				}
			}
		}
...
function OnScoreChange(event) {
	...
	const have_rscores = show_rank_scores && Object.values(session["rScores"]).filter(v => v !== null).length >= 2;
	const have_pscores = show_perf_scores && Object.values(session["pScores"]).filter(v => v !== null).length >= 2;	
	...
	if (have_rscores) {
		// TODO: check the ramk in the sma erow as the event target, if the rank is empty add it based on the scoring rule.
		// Just a stop gap to avoid empty values. The ranks will be reconciled in any case on submission according the scoring 
		// rules. Any submitted values are only used to break ties.
		reconcile_ranks(session, true); 
...
async function OnRowcountChange(event) {
	const num_box = event.target;
	
	// Clean the table (remove any error rows)
	await _table_clear_errors();
...
// We use this to exclude already selected players from any DAL selectors
// Any given player can only be selected once.
...
// Event handler for showing all ranks and scores for debugging
...
function showDiv(which_one) {
	if (which_one === "teams" ) {
		...
		id_show_tie_breakers = id_show_tie_breakers_t;
		...
		id_show_tie_breakers = id_show_tie_breakers_i;
...
// Event handler for the Team Detail visibility checkbox.
// Hides or shows the the TeamPlayersTable
function showhideTeamSwitch(show) {
	const team_switch = $$(id_team_switch);
    const row = getParent(team_switch,"TR");
...
// its in and inserting a TR before it with a message if there's a
...
async function _table_clear_errors(table) {
    if (table == undefined) table = $$(id_tbl_visbl);
    const errorRows = Array.from(table.querySelectorAll('tr.validation-error-row'));
    
    // Start all animations simultaneously
    const animationPromises = errorRows.map(row => removeRowGently(row));
    
    // Wait for the entire group to finish
...
// Database IDs for Django objects are positive integers greater than 0
function isValidID(str) {
    const num = Number(str);
    return Number.isInteger(num) && num > 0;
}

// Ranks must be integers greater than 0
function isValidRank(str) {
    const num = Number(str);
    return Number.isInteger(num) && num > 0;
}

async function isValidSubmission(display_on_form = false) {
    const from_table = $$(id_tbl_players);
    const num_players = Number($$(id_num_players).value);
    const scoring_mode = game_scoring; // Global variable
    const errors = [];
	
	// TODO: Support Team Play mode too
	
    if (display_on_form) await _table_clear_errors(from_table);

    for (let p = 0; p < num_players; p++) {
		const rowErrors = [];
        const wPlayer = getWidget(from_table, name_player, p);
        const wScore  = getWidget(from_table, name_rscore, p);
        const wRank   = getWidget(from_table, name_rank, p);
        const wWeight = getWidget(from_table, name_weight, p);

        const plidVal = getNumberValue(wPlayer, undefined);
        const scoreVal = getNumberValue(wScore, undefined);
        const rankVal = getNumberValue(wRank, undefined);
        const weightVal = getNumberValue(wWeight, undefined);

        // Validation Logic
        if (!plidVal || plidVal === -1) rowErrors.push("No player selected.");
        
        if (scoring_mode === "NO_SCORES") {
            if (!rankVal) rowErrors.push("Rank is required.");
        } else {
            if (scoreVal === undefined || scoreVal === null || isNaN(scoreVal)) {
                rowErrors.push("Score is required.");
            }
        }

        if (weightVal !== undefined && weightVal !== null && (weightVal < 0 || weightVal > 1)) {
            rowErrors.push("Weight must be between 0 and 1.");
        }
		
		if (rowErrors.length > 0) {
            errors.push(`Row ${p + 1}: ${rowErrors.join(' ')}`);

            if (display_on_form && wPlayer) {
                const dataRow = wPlayer.closest('tr');
                const columnCount = dataRow.cells.length;

                const errorRow = document.createElement('tr');
                errorRow.className = 'validation-error-row'; 
                
                const errorCell = document.createElement('td');
                errorCell.colSpan = columnCount;
                errorCell.innerHTML = `<span class="error-icon">⚠️</span> <span class="error-msg">${rowErrors.join(' | ')}</span>`;
                
                errorRow.appendChild(errorCell);
                dataRow.parentNode.insertBefore(errorRow, dataRow);
            }
        }		
    }
...
async function submission_debug(div) {
	// A DRY function.
	// tids are of interest only because we may have injected some DELETE requests if a teams form
	// was loaded and is now being submitted as an indiviudals form.
	console.groupCollapsed(`Submission Integrity Summary for: ${div.id}`);
	...
	// TEAMS
	const report_teams = tinit || ttotal || tids.length || tdels.length
	
	if (report_teams) {  
		console.log('TEAMS')
		if (tinit) console.log(`element: ${tinit.outerHTML}	value: ${tinit.value}`);
		if (ttotal) console.log(`element: ${ttotal.outerHTML}	value: ${ttotal.value}`);
		
		if (tids) {
			console.log(`${tids.length} Team IDs:`);
			for (let tid of tids) console.log(tid.outerHTML);
		} else {
			console.log('No Team IDs');
		}
		...
		if (tdels.length) {
			...
			console.log('No Deleted Team IDs');
		}
	} else {
	...
	// RANKS
	console.log('RANKS')
	console.log(`Rank Management Form:`);
	console.log(`\telement: ${rinit.outerHTML}	value: ${rinit.value}`);
	console.log(`\telement: ${rtotal.outerHTML}	value: ${rtotal.value}`);
	
	console.log(`${rids.length} Rank IDs:`);
    for (let rid of rids) console.log(`\telement: ${rid.outerHTML}	value: ${rid.value}`);
	
	if (rdels.length) {
		console.log(`${rdels.length} Deleted Rank IDs:`);
	    for (let rdel of rdels) console.log(`\telement: ${rdel.outerHTML}	value: ${rdel.value}`);
	} else {
		console.log('No RANKS to delete.');
	}

	console.log(`${ranks.length} Ranks:`);
	for (let rank of ranks) console.log(`\telement: ${rank.outerHTML}	value: ${rank.value}`);

	console.log(`${rscores.length} Rank scores:`);
	for (let rscore of rscores) console.log(`\telement: ${rscore.outerHTML}	value: ${rscore.value}`);
	
	// PERFORMANCE
	console.log('PERFORMANCES')
	console.log(`Performance Management Form:`);
	console.log(`\telement: ${pinit.outerHTML}	value: ${pinit.value}`);
	console.log(`\telement: ${ptotal.outerHTML}	value: ${ptotal.value}`);
	
	console.log(`${pids.length} Performance IDs:`);
    for (let pid of pids) console.log(`\telement: ${pid.outerHTML}	value: ${pid.value}`);

	if (pdels.length) {
		console.log(`${rdels.length} Deleted Performance IDs:`);
	    for (let pdel of pdels) console.log(`\telement: ${pdel.outerHTML}	value: ${pdel.value}`);
	} else {
		console.log('No PERFORMANCES to delete.');
	}
	
	if (pscores.length) {
		console.log(`${pscores.length} Performance scores:`);
	    for (let pscore of pscores) console.log(`\telement: ${pscore.outerHTML}	value: ${pscore.value}`);
	} else {
		console.log('No Performance scores.');
	} 
	...
	console.log(`${pplayers.length} Performance players (the main driver, a powerful Select2 element):`);
    //for (let pplayer of pplayers) console.log(pplayer.outerHTML);
	
	for (let pplayer of pplayers) {
	    // 1. Filter out the placeholder (value="") for an accurate count
	    const actualOptions = Array.from(pplayer.options	).filter(opt => opt.value !== "");
	    
	    // 2. Identify the current live state
	    const selectedOpt = pplayer.options[pplayer.selectedIndex];
	    const isActuallySelected = selectedOpt && selectedOpt.value !== "";
	    
	    // 3. Logic for Label and Preview
	    const statusLabel = isActuallySelected ? "Selected Option:" : "First Option (None Selected):";
	    const previewOpt = isActuallySelected ? selectedOpt : actualOptions[0];
		const displayValue = String(pplayer.value || "None").padEnd(4);

	    console.log(
	        `\tID: ${pplayer.id} | ` +
	        `Options: ${actualOptions.length} | ` +
	        `Value: ${displayValue} | ` +
	        `${statusLabel} ${previewOpt ? previewOpt.outerHTML : 'None'}`
	    );
	}
    ...
    for (let rplayer of rplayers) console.log(`\telement: ${rplayer.outerHTML}	value: ${rplayer.value}`);
	...
	console.log('Summary of PLAYER selections');
	...
	// Print the form data that would be submitted
/*	const formData = new FormData(team_switch.form);
	console.log("%cWould POST:", "color: #3498db; font-weight: bold;");
	const rawBody = new URLSearchParams(formData).toString();
	console.log(rawBody);*/
	
	console.groupCollapsed("%cPOST Data Details", "color: #3498db; font-weight: bold;");

	const formData = new FormData(team_switch.form);
	let maxKeyLength = 0;

	// Find the longest key for perfect alignment
	for (let key of formData.keys()) {
	    if (key.length > maxKeyLength) maxKeyLength = key.length;
	}

	console.log("%cKey".padEnd(maxKeyLength) + " | Value", "text-decoration: underline; font-weight: bold;");

	formData.forEach((value, key) => {
	    let displayValue = value;
	    if (typeof value === 'string') displayValue = value.replace(/\r?\n/g, '\\n');
	    console.log(`${key.padEnd(maxKeyLength)} : ${displayValue}`);
	});

	console.groupEnd();

	// (Don't forget to eventually call console.groupEnd() for your outer group too!)	
	
	// If debugging only we are not submitting, and any selectively disable tids, rids, or
	...
	await isValidSubmission(display_on_form=true);	
}

// A small helper function that given a table will log tot he JS console a summary of its integrity.
// Given the complexity of managing a table of players, performances, rankings and felxible editing
// this little helper can help trace and fix any bugs we may find (as if there are any ;-).
function _table_integrity_debug(table) {
	let from_table = null;
	if (table == undefined) 
		from_table = $$(id_tbl_visbl);
	else if (table.id === "tblIndividualPlay") 
		from_table = $$(id_tbl_players);
	else if (table.id === "tblTeamPlay") { }
	else if (table)
		from_table = table;
		
    console.log(`--- Table Integrity Summary for: ${from_table.id} ---`);
    
    if (from_table.id === id_tbl_players) {
        const num_players = Number($$(id_num_players).value);
        
        // Data collections for cross-row testing
        const rows = [];
        const ridList = [];
        const pidList = [];
        const plidList = [];
        ...
        for (let p = 0; p < num_players; p++) {
            const ridVal   = getNumberValue(getWidget(from_table, name_rid, p), undefined);
            const pidVal   = getNumberValue(getWidget(from_table, name_pid, p), undefined);
            const plidVal  = getNumberValue(getWidget(from_table, name_player, p), undefined);
            const cplidVal = getNumberValue(getWidget(from_table, name_player_copy, p), undefined);
			
			const rscoreVal = getNumberValue(getWidget(from_table, name_rscore, p), undefined);
			const rankVal = getNumberValue(getWidget(from_table, name_rank, p), undefined);
			const weightVal = getNumberValue(getWidget(from_table, name_weight, p), undefined);

			const source = "active";
			const error  = plidVal !== cplidVal;
			
            rows.push({ source, p, plidVal, cplidVal, ridVal, pidVal, rscoreVal, rankVal, weightVal, error });

            // Collect for duplicate checking (ignore -1/undefined)
            if (ridVal !== -1) ridList.push(ridVal);
            if (pidVal !== -1) pidList.push(pidVal);
            if (plidVal !== undefined) plidList.push(plidVal);
        }
		
		const id_trash = from_table.id.replace("tbl", "trash");
		const table_trash = $$(id_trash);
		

		if (table_trash && table_trash.rows.length > 0) {
		    for (let t = 0; t < table_trash.rows.length; t++) {
		        const rowEl = table_trash.rows[t];

		        // 1. Map widgets to a structured object for easy iteration
		        const widgets = {
		            rid:    { el: getWidgetInRow(rowEl, id_rid),         val: (w) => getNumberValue(w, undefined) },
		            pid:    { el: getWidgetInRow(rowEl, id_pid),         val: (w) => getNumberValue(w, undefined) },
		            plid:   { el: getWidgetInRow(rowEl, id_player),      val: (w) => getNumberValue(w, undefined) },
		            cplid:  { el: getWidgetInRow(rowEl, id_player_copy), val: (w) => getNumberValue(w, undefined) },
		            rscore: { el: getWidgetInRow(rowEl, id_rscore),      val: (w) => getNumberValue(w, undefined) },
		            rank:   { el: getWidgetInRow(rowEl, id_rank),        val: (w) => getNumberValue(w, undefined) },
		            weight: { el: getWidgetInRow(rowEl, id_weight),      val: (w) => getNumberValue(w, undefined) }
		        };

		        // 2. Group these by their detected Form Index
		        const identityGroups = {};
		        
		        Object.entries(widgets).forEach(([key, config]) => {
		            const fIdx = getFormNumber(config.el);
		            // Treat null/undefined index as a special 'missing' group
		            const groupKey = fIdx !== null ? fIdx : "missing";
		            
		            if (!identityGroups[groupKey]) {
		                identityGroups[groupKey] = { fIdx: fIdx, data: {} };
		            }
		            identityGroups[groupKey].data[key] = config.val(config.el);
		        });

		        // 3. Process the Groups
		        const uniqueIndices = Object.keys(identityGroups);
		        const source = "trash";
		        const error = uniqueIndices.length > 1;

		        uniqueIndices.forEach(idxKey => {
		            const group = identityGroups[idxKey];
		            
		            // Push a row to the collection
		            rows.push({
		                source: source,
		                p: group.fIdx,
		                plidVal:   group.data.plid,
		                cplidVal:  group.data.cplid,
		                ridVal:    group.data.rid,
		                pidVal:    group.data.pid,
		                rscoreVal: group.data.rscore,
		                rankVal:   group.data.rank,
		                weightVal: group.data.weight,
						error:     error
		            });
		        });
		    }
        ...
        // --- TEST 1: Shadow Consistency ---
        const mismatchRows = rows.filter(r => r.plidVal !== r.cplidVal);
        const test1Status = mismatchRows.length === 0 ? "✅ PASSED" : `❌ FAILED (Rows: ${mismatchRows.map(r => r.p).join(', ')})`;
        console.log(`1. Shadow Consistency (plid === plid_copy): ${test1Status}`);

        // --- Helper for Duplicate Logic ---
		const getDuplicates = (list) => list.filter((item, index) => 
		    item !== null && item !== undefined && item !== '' && 
		    list.indexOf(item) !== index
		);        const formatApiLink = (type, id) => `%c[View ${type} ${id}]%c (yoursite.com/api/${type}/${id}/)`;

        // --- TEST 2, 3, 4: Duplicate Detection ---
        const checkDuplicates = (name, list, apiType) => {
            const dups = [...new Set(getDuplicates(list))];
            if (dups.length === 0) {
                console.log(`${name}: ✅ PASSED (No duplicates)`);
            } else {
                console.log(`${name}: ❌ FAILED (Duplicates found: ${dups.join(', ')})`);
                dups.forEach(id => {
                    // Clicking this in most consoles will navigate or allow copy
                    console.log(formatApiLink(apiType, id), "color: blue; text-decoration: underline;", "");
                });
            }
        };

        checkDuplicates("2. Rank ID Uniqueness", ridList, "rank");
        checkDuplicates("3. Performance ID Uniqueness", pidList, "performance");
        checkDuplicates("4. Player Selection Uniqueness", plidList, "player");

		// TODO: Summarise management forms for Ranks and Performances
		
        // --- Detailed Row Log (Optional / Collapsed) ---
        console.groupCollapsed("Detailed Row Data");
		// Define column widths
	    const w = { src: 10, p: 6, pl: 8, rid: 8, pid: 8, sc: 7, rk: 5, wt: 7 };

	    // Helper to pad strings for consistent columns
	    const pad = (val, width) => (val === undefined ? "-" : val).toString().padEnd(width);
	    ...
	    console.log(`%c${header}`, "font-weight: bold; border-bottom: 1px solid black;");
	    ...
	    rows.forEach(r => {
	        const rowStr = 
	            ...
	            pad(r.ridVal, w.rid) + 
	            pad(r.pidVal, w.pid) + 
	            pad(r.rscoreVal, w.sc) + 
	            pad(r.rankVal, w.rk) + 
	            pad(r.weightVal, w.wt);
	        
	        // You can even color-code specific rows if they have issues
	        const color = r.error ? "color: red;" : "color: inherit;";
	        console.log(`%c${rowStr}`, color);
	    });
		console.log(`%c${header}`, "color: transparent; border-top: 1px solid black;");		
        console.groupEnd();
    } 
    else if (from_table.id === id_tbl_teams) { /* ... */ }
    else { console.log(`Unsupported table id.`); }
}

// On page load we build the tables needed and it may be the case that the DAL/select2 
// widgets may not have finished initialising and not return the seleed value even 
// if we just set it! So we chekc that they are initialised and if not we wait until 
// they all are before checking table integrity.  
function table_integrity_debug(table) {
	let from_table = null;
	if (table == undefined) 
		from_table = $$(id_tbl_visbl);
	else if (table.id === "tblIndividualPlay") 
	...
	const selectors = from_table.querySelectorAll('select[data-autocomplete-light-function="select2"]');
    
    // 1. Quick check: Are they ALL already initialized?
	const allReady = Array.from(selectors).every(el => {
	    // Does the Select2 instance exist on this element?
	    const hasS2Data = !!$(el).data('select2');
	    const hasValue = el.value !== "" && el.value !== null;
	    return hasS2Data && hasValue;
	});
	
    if (allReady) {
        // SYNCHRONOUS PATH: Everything is ready, run now.
        _table_integrity_debug(from_table);
    } 
	else {
	    // 2. ASYNCHRONOUS PATH: At least one is still initializing.
		console.log(`Table pending init: waiting on DAL/select2 elements to initialise.`);		
	    
		const readinessPromises = Array.from(selectors).map(el => {
		    // Check 1: Is the Select2 instance already there?
		    // Check 2: Does it have a value (meaning AJAX is done)?
		    if ($(el).data('select2') && el.value !== "") {
		        return Promise.resolve();
		    }

		    return new Promise((resolve) => {
		        // DAL-specific event: fires when it finishes setup and initial AJAX
		        $(el).one('autocompleteLight:initialized', () => {
		            resolve();
		        });

		        // Safety fallback: if it's already initialised but somehow missed the event
		        if ($(el).data('select2')) {
		            // A tiny timeout to see if AJAX fills the value
		            setTimeout(resolve, 100); 
		        }

		        // Global safety: 2 seconds max
		        setTimeout(resolve, 2000);
		    });
		});
	
	    Promise.all(readinessPromises).then(() => _table_integrity_debug(from_table));
	}
}

function debug_visible_table() { table_integrity_debug(); }
