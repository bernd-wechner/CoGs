...
function InitControls(options, exempt) {
	...
	// the server provides in this case.
	...
	if (!ex.includes("order")) $('#order').val(dval(options.order, "default"));
	...
	if (!ex.includes("count_cross_league_plays")) $('#chk_count_cross_league_plays').prop('checked', dval(options.count_cross_league_plays, true));
...
function got_new_leaderboards() {
	if (REQUEST.readyState === 4 && REQUEST.status === 200){
		...
		// Set some globals to support sensible layout and rendering
...
function cancelRefetch() {
	if ("leaderboards" in waiting_for) {
		delete waiting_for["leaderboards"]; 
	
		// Hide all the reloading icons we're supporting
		$("#reloading_icon").css("visibility", "hidden");
		$("#reloading_icon_advanced").css("visibility", "hidden");
	
		// Enable all the submission buttons
		enable_submissions(true);
	}	
}
