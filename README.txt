...
but it can be imporved with each new effort. So if you're stating out, note anything that you 
...
and the PyDev support guys suggested the above which worked a breeze.
	...
	a) DON'T DO THIS. GOT b) ;-)
	   If really wanting to build a Django project from scratch, super clean do this:
		
		i) Create a new Pydev Django Project in Eclipse (CoGs)
		ii) Create a new Django App in Eclipse (Leaderboards)
		iii) Find the CoGs code and copy individual files as needed into these projects which includes but is not limited to:
			CoGs/settings.py
			CoGs/urls.py
			CoGs/wsgi.py
			...
			I did this once in a massive migration from old code base to new wanting a real clean break clear of all old lint.
			
	b) If you're happy to use the current project (VERY recommended as a) is a lot of work) then:
		i) Work out where you want it to live. On a Linux system recommend ~/workspace
		ii) Fetch it from github with: 
		iii) Open the Eclipse project file in Eclipse (Work out how to do that and update this!)		
...
4) Try it out
	...
	a) Open the CoGs project in Eclips
	b) Right click the project then click "Debug As..." the "PyDev: Django". 
	c) In your Console panel you should see something like:
	...
	d) In your favourite web browser open http://127.0.0.1:8000/ and play around.
...
Produces a rich dcoumentation site under index.html including a better schema diagram, but you cna't move things around, it's well layed out but fixed in place.
