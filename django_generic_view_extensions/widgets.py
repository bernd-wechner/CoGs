...
class DateTimeSelector(DateTimeInput):
    ...
    TODO: A stub I'd like to see implemented. Be nice if the DateTimeInput widget could come in a 
          ...
          and so this widget should have some javascript that checks if tehse have been loaded I guess, 
...
class OrderingWidget(SelectMultiple):
    ...
    :param model: The model that must provide a 'order_options' attribute. 
                  Being a list of filter criteria for which values can be sought/provided
        ...
        header = ("<style>"
                   ...
                   "<script>"
                        ...
                        # TODO: If moving a nodir up above the nodir block switch to up
                            "if (btn.innerHTML == String.fromCharCode({up}))" 
                                "btn.innerHTML = '&#{down};';" 
                            "else if (btn.innerHTML == String.fromCharCode({down}))" 
                                "btn.innerHTML = '&#{nodir};';" 
                            "else if (btn.innerHTML == String.fromCharCode({nodir}))" 
                                "btn.innerHTML = '&#{up};';" 
                            " else " 
                                "btn.innerHTML = '&#{up};';" 
                        "}}"

# DO this in the template first to debug a URL creator then capture here.                        
#                         "function CollectField(index, value) {fields.push($(value).text());}"
#                         "function CollectDir(index, value) {dirs.push($(value).text());}"
# 
#                         "function ordering_url_opts() {"
#                             "fields = [];  dirs = [];"
#                             "$('#{id}').find('span[id^=\"ordering_field\"]').each(CollectField);"
#                             "$('#{id}').find('span[id^=\"ordering_dir\"]').each(CollectDir);"
#                             "return URL"
#                         "}"
